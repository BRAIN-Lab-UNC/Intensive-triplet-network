...
GAP = 100  #100-200 days, 200-300 days, etc.
loss_coeff = [0.05, 0.005, 0.005, 40, 15]   #0.1*triplet_loss + 0.1*age_loss + recon_loss  + pcc_loss
...
if __name__ == '__main__':
    ...
    save_index = 0  #add more layer in residualblock
    for RT in range(opt.RepTime):   #Repteat Experiments
        ...
        save_index += 1
        exp_dir = './ExpResults/Exp' + str(save_index)+ '_unit_count_'+str(opt.unit_count)+'ReLU2_pccLoss'
        ...
        with torch.no_grad():
            ...
            for tr_ite in range(len(te_FCs)):
                ...
                if (te_RealID not in tested_IDs):
                    ...
                    #min_value = torch.min(recon_test) 
                    ...
                    for co_ite in range(tr_ite+1, len(te_FCs), 1):
                        ...
                        if(te_RealID_co == te_RealID and te_Day_co != te_Day):
                            ...
                            np.savetxt(exp_dir+'/'+str(te_RealID_co)+'_'+str(te_one_Day_co)+'_recon.txt',FC_recon_co)
