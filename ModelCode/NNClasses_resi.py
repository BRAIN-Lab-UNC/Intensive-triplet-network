...
class ReconNet(nn.Module): 
    def __init__(self,input_size):
        ...
        self.layer1 = nn.Sequential(nn.Linear(opt.latent_dim,input_size), nn.BatchNorm1d(input_size), nn.Tanh())  #nn.Linear(opt.latent_dim,opt.latent_dim)  nn.LeakyReLU(0.3)
...
class ResidualBlock(nn.Module):
    ...
    def forward(self, inputs):
        ...
        return {0: x2+x, 1: task_ids}  #{0: x2 + x1, 1: task_ids}
